/**
 * ENVOY — Public portfolio renderer (audit findings S1 / S9 / PF4)
 *
 * Presentational components for the public `/p/[slug]` page. They receive
 * ONLY the public-safe projections (`PublicPortfolioProfile` /
 * `PublicPortfolioSite`); the canonical profile and owner ids never reach
 * this layer. Rendered server-side in cloud mode; rendered client-side from
 * the (projected) localStorage record in demo mode.
 */
...
export default function PublicPortfolioViews({
  profile,
  site,
}: {
...
function MinimalThemeView({ profile, site }: { profile: PublicPortfolioProfile; site: PublicPortfolioSite }) {
...
function DeveloperThemeView({ profile, site }: { profile: PublicPortfolioProfile; site: PublicPortfolioSite }) {
  const visible = (type: string) => site.sections.find(s => s.type === type)?.visible !== false
...
function BoldThemeView({ profile, site }: { profile: PublicPortfolioProfile; site: PublicPortfolioSite }) {
  const visible = (type: string) => site.sections.find(s => s.type === type)?.visible !== false
